...
void createIsolate() async {
  ...
  await Isolate.spawn((message) {// 在这里面创建 child子Isolate进程
    ...
    childReceivePort.listen((message) { // 通过 child子Isolate的ReceivePort监听 child子SendPort；
      print("[Spawn Thread child isolate] message: $message");
    });
    mainSendPort.send([0, childSendPort]);// 在 child子Isolate里面，通过 main主 SendPort 向 main主Isolate发送message消息；
    sleep(const Duration(seconds: 3));// 当前Isolate，也就是 child子Isolate 休眠 3s；
    mainSendPort.send([1, "[child isolate] finish"]);// 在 child子Isolate里面，通过 main主 SendPort 向 main主Isolate发送message消息；
