...
class SearchFunction extends State<CupertinoPageRouteAndTheme2> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: OutlinedButton(
        onPressed: () {
          Map argumentsMap = {"title": "iOS风格下 去 注册的第3个 路由页面", "aid": 61};
          print(argumentsMap);
          Navigator.pushNamed(context, "/third", arguments: argumentsMap);
        },
        child: const Text("命名路由传值：去 注册的第3个 路由页面",
            style: TextStyle(
                fontSize: 14, color: Colors.pink, fontStyle: FontStyle.normal)),
      ),
    );
  }
}
